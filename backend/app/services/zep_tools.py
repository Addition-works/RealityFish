...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            f"Query: {self.query}",
            f"Research context: {self.simulation_requirement}",
            f"\n### Data Statistics",
        ...
        if self.semantic_facts:
            text_parts.append(f"\n### [Key Facts] (cite these directly in the report)")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        """转换为文本格式（完整版本，不截断）"""
        text_parts = [
            f"## Broad Search Results (Panorama View)",
            f"Query: {self.query}",
            f"\n### Statistics",
        ...
        if self.active_facts:
            text_parts.append(f"\n### [Active Facts] (cite these directly)")
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            f"**Interviews Conducted:** {self.interviewed_count} / {self.total_agents} agents",
            "\n### Selection Rationale",
...
class ZepToolsService:
    ...
    def __init__(
        ...
        self._trace = TraceLogger("step5_zep_tools", simulation_id) if simulation_id else None
    ...
    def insight_forge(
        ...
        if self._trace:
            self._trace.log("INTERMEDIATE", "sub_queries", sub_queries)
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are an expert question analyst. Your task is to decompose a complex question into multiple sub-questions that can be independently investigated in a knowledge graph of real-world data.

Requirements:
1. Each sub-question should be specific enough to find relevant entities, behaviors, or events in the data
2. Sub-questions should cover different dimensions of the original question (who, what, why, how, when, where)
3. Sub-questions should be relevant to the research context
...
Return a JSON list of sub-questions in English."""
        ...
        try:
            ...
            return [
                query,
                f"Key participants and entities involved in: {query}",
    ...
    def panorama_search(
        ...
        if self._trace:
            self._trace.log("INPUT", "panorama_search", {"query": query})
        ...
        if self._trace:
            self._trace.log(
                "OUTPUT",
                "panorama_counts",
                {"active_count": result.active_count, "historical_count": result.historical_count},
            )
        return result
    ...
    def interview_agents(
        ...
        if self._trace:
            self._trace.log(
                "INPUT",
                "interview_agents",
                {"interview_requirement": interview_requirement, "simulation_id": simulation_id},
            )
        ...
        if self._trace:
            selected_names = [
                a.get("realname", a.get("username", f"Agent_{i}"))
                for i, a in enumerate(selected_agents)
            ]
            self._trace.log(
                "INTERMEDIATE",
                "selected_agents",
                {"names": selected_names, "indices": selected_indices},
            )
        ...
        try:
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may be shut down."
            return result
        except Exception as e:
        ...
        if self._trace:
            self._trace.log("OUTPUT", "interviewed_count", result.interviewed_count)
        return result
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are an expert interview planner. Your task is to select the most suitable agents for interview based on the interview requirements.
...
Please select up to {max_agents} agents best suited for interview, and explain your reasoning."""
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional journalist/interviewer. Generate 3-5 in-depth interview questions based on the requirements.
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = f"""You are a professional editor. Based on the responses from multiple interviewees, generate an interview summary.

Summary requirements:
1. Distill the main viewpoints from all parties
2. Identify consensus and disagreements
3. Highlight valuable quotes
4. Be objective and neutral, don't favor any side
...
Please generate an interview summary."""
        ...
        try:
            ...
            # 降级：简单拼接
            return f"Interviewed {len(interviews)} participants: " + ", ".join([i.agent_name for i in interviews])
